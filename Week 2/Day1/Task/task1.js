...
const analyzeUserData = (users) => {
    const simulatedCurrentDate = new Date("2024-05-17T00:00:00");

    const isWithinLastWeek = (timestamp) => {
        const oneWeekAgo = new Date(simulatedCurrentDate);
        oneWeekAgo.setDate(oneWeekAgo.getDate() - 7);
        return new Date(timestamp) > oneWeekAgo;
    };

    const isPopular = (post) => post.likes >= 10;

    const result = users
        .map(user => ({
            ...user,
            posts: user.posts.filter(post => isWithinLastWeek(post.timestamp))
        }))
        .filter(user => user.posts.length > 0)
        .map(user => ({
            ...user,
            posts: user.posts.filter(isPopular)
        }))
        .reduce((acc, user) => {
            const totalLikes = user.posts.reduce((sum, post) => sum + post.likes, 0);
            return {
                activeUsers: acc.activeUsers + 1,
                totalPopularPosts: acc.totalPopularPosts + user.posts.length,
                totalLikes: acc.totalLikes + totalLikes
            };
        }, { activeUsers: 0, totalPopularPosts: 0, totalLikes: 0 });

    const averageLikesPerUser = result.activeUsers > 0
        ? result.totalLikes / result.activeUsers
        : 0;
